impl CodeGen {
  unsafe fn build_closure(
    ...
    let block =
      LLVMAppendBasicBlockInContext(self.context, func, c_str!("fn_block"));
    ...
    Ok(match expr.kind {
      ...
      ExpressionKind::U8Rec(_, _, contents) => {
        ...
        let count_thunk = {
          self.build_closure(
            ...
            |_, func_builder, env_param, _| {
              ...
              let count =
                LLVMBuildLoad(func_builder, count_ptr, c_str!("count"));
              Ok(count)
            },
          )?
        };
        let acc_thunk = {
          ...
          let iter_env_ids =
            iter_env_vec.iter().map(|(id, _)| *id).collect::<Vec<_>>();
...
pub fn codegen(ast: Expression, file: *const i8) {
  unsafe {
    ...
    let noalias =
      LLVMCreateStringAttribute(context, c_str!("noalias"), 7, c_str!(""), 0);
    ...
    let main_block =
      LLVMAppendBasicBlockInContext(context, main_func, c_str!("main"));
